...
{
  //oled配置
  bsp_oled_write_cmd(OLED_WRITE_ADDR,TURN_OFF_CMD);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0xAE);//关显示
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X20);//低列地址
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X10);//高列地址
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XB0);//
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XC8);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X00);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X10);
   //设置行显示的开始地址(0-63)  
  //40-47: (01xxxxx)  
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X40);
   //设置对比度  
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X81);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X22);//这个值越大，屏幕越亮(和上条指令一起使用)(0x00-0xff) 

  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XA1);//0xA1: 左右反置，  0xA0: 正常显示（默认0xA0）
 //0xA6: 表示正常显示（在面板上1表示点亮，0表示不亮）  
  //0xA7: 表示逆显示（在面板上0表示点亮，1表示不亮）
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XA6); 

  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XA8);//设置多路复用率（1-64） 
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X3F);//（0x01-0x3f）(默认为3f)
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XA4);
  //设置显示抵消移位映射内存计数器  
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XD3);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X00);
  //设置显示时钟分频因子/振荡器频率 
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XD5);
  //低4位定义显示时钟(屏幕的刷新时间)（默认：0000）分频因子= [3:0]+1  
  //高4位定义振荡器频率（默认：1000） 
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XF0);
  //时钟预充电周期  
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XD9);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X22);
  //设置COM硬件应脚配置  
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XDA);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X12);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XDB);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X20);
  //电荷泵设置（初始化时必须打开，否则看不到显示）
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X8D);
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0X14);
  //开显示
  bsp_oled_write_cmd(OLED_WRITE_ADDR,0XAF);
  //清屏
  bsp_oled_clear();
}
...
void bsp_oled_update_screen(void)
{
    ...
    {
        bsp_oled_write_cmd(OLED_WRITE_ADDR,0xb0+line_index);
        bsp_oled_write_cmd(OLED_WRITE_ADDR,0x00);
        bsp_oled_write_cmd(OLED_WRITE_ADDR,0x10);
        
        bsp_oled_write_long_data(OLED_WRITE_ADDR,&g_oled_buffer[SSD1306_WIDTH * line_index],SSD1306_WIDTH);
    }
}
